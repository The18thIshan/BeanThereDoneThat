...
func _update_camera(delta):
	...
	$Camera2D.offset.x = lerp(camera.offset.x, target_cam_offset, CAM_SMOOTHING * delta)
...
func player():
	pass # This does nothing, it's just a "tag" so the level knows this is the player
